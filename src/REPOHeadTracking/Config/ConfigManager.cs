    ...
    internal sealed class ConfigManager
    {
        ...
        public void Initialize(ConfigFile config)
        {
            ...
            InvertYaw = config.Bind(
                "Sensitivity", "InvertYaw", false,
                "Invert horizontal (left/right) head rotation");

            // R.E.P.O.'s camera pitches the opposite way to the tracker's convention,
            // so the default is inverted here rather than in the shared pipeline.
            InvertPitch = config.Bind(
                "Sensitivity", "InvertPitch", true,
                "Invert vertical (up/down) head rotation");
